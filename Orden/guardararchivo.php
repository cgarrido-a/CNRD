...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (move_uploaded_file($archivoSubido['tmp_name'], $rutaArchivo)) {
        if (strpos($idVoluntario, 'c-') === 0) {
            ...
            if ($r) {
                header("Location: editarUsuario.php?id=$resto&success=Archivo subido correctamente");
            } else {
                
                header("Location: editarUsuario.php?id=$resto&error=Error al actualizar la base de datos");
            }
        } else {
            $resultado = Usuario::actualizarArchivo($idVoluntario, $campo, $rutaArchivo);
            if ($resultado) {
                header("Location: verVoluntario.php?id=$idVoluntario&success=Archivo subido correctamente");
            } else {
                header("Location: verVoluntario.php?id=$idVoluntario&error=Error al actualizar la base de datos");
            }
        }
    } else {
        header("Location: verVoluntario.php?id=$idVoluntario&error=Error al subir el archivo");
    }
}
